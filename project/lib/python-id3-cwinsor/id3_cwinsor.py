...
class ID3(object):
    ...
    def project_columns(self, data, columns_to_project):
    ...
    def get_class_labels(self, data, target_attribute):
        rows = data['rows']
    ...
    def avg_entropy_w_partitions(self, data, splitting_att, target_attribute):
    ...
    def fit(self, data, remaining_atts, target_attribute):

        if (data[target_attribute].nunique() == 1):
            self.decision_attribute = data[target_attribute][0]
            self.is_leaf = True
            return self
